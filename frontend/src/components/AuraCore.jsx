...
const AuraCore = () => {
  ...
  return (
    ...
    >
      <div
        className={`absolute inset-0 rounded-full bg-gradient-to-tr from-purple-600 via-pink-400 to-indigo-500 blur-3xl opacity-30 transition-all duration-1000 ${isHovered ? 'scale-125 opacity-55' : 'scale-100'
          }`}
      />

      <div
        className={`absolute w-[110%] h-[110%] rounded-full bg-gradient-to-r from-neon-orchid/30 to-purple-500/20 blur-2xl transition-all duration-700 ${isHovered ? 'animate-pulse-glow scale-105 opacity-80' : 'opacity-40'
          }`}
      />

      <div
        className={`absolute inset-0 rounded-full border border-white/10 backdrop-blur-xl transition-all duration-1000 ${isHovered ? 'scale-105 border-white/20 rotate-45' : 'scale-95'
          }`}
      />

      <div
        className={`absolute w-60 h-60 md:w-64 md:h-64 animate-orb-morph overflow-hidden border border-white/25 glass-panel transition-all duration-700 bg-gradient-to-tr from-pink-300/30 via-purple-400/20 to-indigo-300/20 ${isHovered
      ...
      >
        ...
        <div className={`absolute inset-12 rounded-full border border-pink-200/20 transition-all duration-700 ${isHovered ? 'scale-110 border-pink-200/40' : 'scale-100'
          }`} />
        <div className={`absolute inset-20 rounded-full border border-purple-300/25 transition-all duration-700 ${isHovered ? 'scale-90 border-purple-300/50' : 'scale-100'
          }`} />
      </div>
      ...
      <div className="absolute flex flex-col items-center justify-center text-center z-10 select-none">
        ...
        <span className={`text-sm sm:text-base font-serif font-semibold tracking-wide transition-all duration-300 ${sosStatus === 'idle' ? 'text-white group-hover:text-neon-orchid' :
        ...
        <div className="mt-3 flex items-center justify-center">
          <span className="relative flex h-2 w-2">
            <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${sosStatus === 'success' ? 'bg-emerald-400' :
                sosStatus === 'failed' ? 'bg-red-500' :
                  'bg-neon-orchid'
              }`}></span>
            <span className={`relative inline-flex rounded-full h-2 w-2 ${sosStatus === 'success' ? 'bg-emerald-400' :
                sosStatus === 'failed' ? 'bg-red-500' :
                  'bg-neon-orchid'
              }`}></span>
          </span>
