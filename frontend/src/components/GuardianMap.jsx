...
export default function GuardianMap() {
    ...
    return (
        <div
            ...
            className={`glass-panel glass-panel-glow relative flex flex-col p-8 rounded-3xl select-none transition-all duration-700 w-full ${isMapHovered
        ...
        >
            <div className={`absolute -top-12 left-1/4 w-1/2 h-20 bg-gradient-to-b from-pink-300/10 via-purple-500/5 to-transparent blur-xl transition-opacity duration-700 ${isMapHovered ? 'opacity-100' : 'opacity-40'}`} />
            ...
            <div className={`absolute inset-0 rounded-3xl border border-transparent transition-all duration-700 pointer-events-none ${isMapHovered ? 'bg-gradient-to-r from-neon-orchid/20 via-pink-300/10 to-indigo-500/20 [mask-image:linear-gradient(to_bottom,white,transparent)]' : ''}`} />
        </div>
    );
}